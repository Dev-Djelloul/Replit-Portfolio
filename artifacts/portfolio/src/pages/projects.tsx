...
import { fallbackProjects, filterProjects, getProjectStats, sortProjectsByProjectNumber } from "@/data/projects";
...
export default function Projects() {
  ...
  const projectList = sortProjectsByProjectNumber(
        ...
        <div className="min-h-[500px]">
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-[500px] rounded-xl bg-card border border-border/50 animate-pulse" />
              ))}
            </div>
